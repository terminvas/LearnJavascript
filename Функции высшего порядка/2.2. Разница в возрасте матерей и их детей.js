...
// Середнє арифметичне елементів масива
function average(array) {
   function plus(a, b) { return a + b; }
   return array.reduce(plus) / array.length;
}

// Сортуємо у порядку спадання років народження (не обов'язково)
//ancestry.sort((a, b) => b.born - a.born);

// Масив різниць років
tempArr = [];
ancestry.forEach((a, _index, array) => {
   // Перший індекс, коли ім'я матері із поля "mother" співпадає з ім'ям з поля "name"
   let indexBornMother = array.findIndex((b) => a.mother == b.name);
   // Якщо такий індекс є (!= -1), додаємо різницю років в масив
   if (indexBornMother != -1) tempArr.push(a.born - array[indexBornMother].born);
});

console.log(Math.round(average(tempArr) * 10) / 10);
